    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', width: '100%', fontFamily: "'Poppins', 'Segoe UI', sans-serif", background: COLORS.cream, overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden', position: 'relative' }}>
        <div ref={canvasRef} onClick={handleCanvasClick} style={{ flex: 1, position: 'relative', overflow: 'auto', backgroundImage: 'radial-gradient(circle, #E3D7C5 1px, transparent 1px)', backgroundSize: '22px 22px', backgroundColor: COLORS.cream }}>
          ...
          {/* Floating button to open the asset drawer — always reachable, never steals canvas space when closed */}
          {!paletteOpen && (
            <button
              onClick={() => setPaletteOpen(true)}
              style={{
                position: 'absolute', bottom: 18, right: 18, width: 52, height: 52, borderRadius: '50%',
                border: 'none', background: COLORS.accent, color: '#fff', display: 'flex', alignItems: 'center',
                justifyContent: 'center', boxShadow: '0 4px 14px rgba(194,69,107,0.4)', cursor: 'pointer', zIndex: 20,
              }}
              title="Add icon, shape, or image"
            >
              <Plus size={24} />
            </button>
          )}
