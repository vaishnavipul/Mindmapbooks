...
import {
  Search, Mic, Type, Circle, Square, Diamond, X,
...
/* ============================================================
   CANONICAL BOARD ITEM SCHEMA — single source of truth.
   Every board item, regardless of type (text/shape/emoji/image),
   ALWAYS has every one of these keys. Fields that don't apply to
   a given type are simply null. This guarantees:
     1. Every item sent to Supabase has identical keys, so bulk
        inserts never hit "PGRST102: All object keys must match."
     2. Adding a new field (e.g. for a future item type, or for
        bulk-uploaded images) only requires editing ONE place —
        add it to ITEM_DEFAULTS, and every item automatically
        gets it, and every save automatically includes it.
   If you add a new field here, also add the matching column to
   the board_items table in Supabase (same name, nullable).
============================================================ */
const ITEM_DEFAULTS = {
  id: null,
  board_id: null,
  type: null,       // 'text' | 'shape' | 'emoji' | 'image'
  x: 0,
  y: 0,
  w: 100,
  h: 70,
  label: null,
  glyph: null,       // emoji glyph
  shape: null,       // 'rect' | 'circle' | 'diamond'
  color: null,       // shape fill color
  src: null,         // image URL
  highlight: false,  // text highlight toggle
  audio_clip_url: null, // optional short audio clip attached to this item
};

/** Builds a complete, save-safe item by filling any missing keys
 * with their default (null/0/false), so callers can pass only the
 * fields relevant to that item's type. */
function makeItem(partial) {
  return { ...ITEM_DEFAULTS, ...partial };
}

/** Strips an in-memory item down to exactly the canonical key set,
 * in a fixed order, ready for a Supabase insert. Using this in ONE
 * place (saveBoard) means every item — no matter how it was created
 * or what extra in-memory fields it might carry — gets normalized
 * identically before hitting the database. */
function toSavePayload(item, boardId) {
  return {
    id: item.id,
    board_id: boardId,
    type: item.type,
    ...
    audio_clip_url: item.audio_clip_url ?? null,
  };
}
...
/* ============================================================
   PERSISTENT AUDIO — one shared <audio> element lives at the
   root level (inside MindBoardRoot) so playback survives
   navigation between the book detail screen and the mind map
   screen. AudioPlayer (full controls) and MiniAudioBar (compact,
   shown on the mind map) both read/write this same state.
============================================================ */
function usePersistentAudio() {
  ...
  const loadBook = (book) => {
    if (currentBook?.id === book.id) return; // already loaded, don't restart
    setCurrentBook(book);
    setPlaying(false);
    setCurrentTime(0);
    if (audioRef.current) { audioRef.current.src = book.audio_url || ''; }
  };
  const togglePlay = () => {
    const audio = audioRef.current;
    if (!audio || !currentBook?.audio_url) return;
  ...
  return { audioRef, currentBook, playing, currentTime, duration, rate, loadBook, togglePlay, skip, seekTo, cycleRate };
}
...
/** Compact persistent mini-bar — shown on the mind map screen so the
 * book's audio is still controllable (and audibly confirmed playing)
 * while the user is mapping, without taking much screen space. */
function MiniAudioBar({ audio, book }) {
  if (!book.audio_url) return null;
  return (
    <div style={{
      display: 'flex', alignItems: 'center', gap: 10, padding: '8px 14px',
      background: COLORS.ink, color: '#fff', flexShrink: 0,
    }}>
      <button onClick={audio.togglePlay} style={{
        width: 30, height: 30, borderRadius: '50%', border: 'none', background: COLORS.accent, color: '#fff',
        display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0,
      }}>
        {audio.playing ? <Pause size={14} /> : <Play size={14} />}
      </button>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 11.5, fontWeight: 700, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{book.title}</div>
        <input type="range" min={0} max={audio.duration || 0} value={audio.currentTime} onChange={(e) => audio.seekTo(Number(e.target.value))}
          style={{ width: '100%', height: 3, accentColor: COLORS.accent }} />
      </div>
      <span style={{ fontSize: 10.5, opacity: 0.8, flexShrink: 0 }}>{fmtTime(audio.currentTime)} / {fmtTime(audio.duration)}</span>
    </div>
  );
}

/** Draggable, resizable floating window for viewing the book's PDF
 * while mind-mapping. Renders the PDF in an <iframe>; drag the header
 * to move, drag the bottom-right corner to resize. Closes via the X. */
function DraggablePdfWindow({ pdfUrl, onClose }) {
  ...
  const startDrag = (e, mode) => {
    ...
    dragRef.current = { mode, startX: clientX, startY: clientY, origPos: { ...pos }, origSize: { ...size } };
  ...
  const onDragMove = (e) => {
    ...
    if (dragRef.current.mode === 'move') {
      setPos({ x: Math.max(0, dragRef.current.origPos.x + dx), y: Math.max(0, dragRef.current.origPos.y + dy) });
    } else {
      setSize({ w: Math.max(220, dragRef.current.origSize.w + dx), h: Math.max(260, dragRef.current.origSize.h + dy) });
    }
  };
  const onDragEnd = () => {
  ...
  return (
    <div style={{
      position: 'absolute', left: pos.x, top: pos.y, width: size.w, height: size.h, zIndex: 40,
      background: '#fff', borderRadius: 12, boxShadow: '0 8px 28px rgba(0,0,0,0.25)', border: `1px solid ${COLORS.line}`,
      ...
      <div
        ...
        style={{
          ...
          background: COLORS.sand, cursor: 'grab', flexShrink: 0, touchAction: 'none',
        }}
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 700, color: COLORS.ink }}>
          <FileText size={14} /> PDF
        </div>
        <button onClick={onClose} style={{ background: 'none', border: 'none', color: '#998F80', cursor: 'pointer', display: 'flex' }}>
          <X size={16} />
        </button>
      </div>
      <iframe src={pdfUrl} title="Book PDF" style={{ flex: 1, border: 'none', width: '100%' }} />
      <div
        onMouseDown={(e) => startDrag(e, 'resize')}
        onTouchStart={(e) => startDrag(e, 'resize')}
        style={{
          position: 'absolute', right: 0, bottom: 0, width: 22, height: 22, cursor: 'nwse-resize',
          touchAction: 'none', display: 'flex', alignItems: 'flex-end', justifyContent: 'flex-end', padding: 3,
        }}
      >
        <div style={{ width: 10, height: 10, borderRight: `2.5px solid ${COLORS.line}`, borderBottom: `2.5px solid ${COLORS.line}` }} />
      </div>
    </div>
  );
}

/** Modal for attaching a short voice clip to a single mind map item.
 * Two paths: record live via the microphone (MediaRecorder API), or
 * upload an existing short audio file. Either way, the result is
 * uploaded to Supabase Storage and the resulting URL is attached to
 * the item via onAttach. Recording is capped at 30 seconds — this is
 * meant for a quick spoken note per idea, not the full book. */
function ClipRecorderModal({ item, auth, onAttach, onRemove, onClose }) {
  const [mode, setMode] = useState('choose'); // choose | recording | reviewing | uploading
  ...
  const saveClip = async () => {
    ...
    setError('');
    try {
      const ext = recordedBlob.type.includes('webm') ? 'webm' : (recordedBlob.name?.split('.').pop() || 'mp3');
      const path = `${auth.user.id}/clips/${Date.now()}_clip.${ext}`;
      const url = await sb.uploadFile(auth.token, 'board-images', path, recordedBlob);
      onAttach(item.id, url);
      onClose();
    } catch (err) {
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: '#fff', borderRadius: 16, padding: 22, width: '100%', maxWidth: 320, boxShadow: '0 10px 40px rgba(0,0,0,0.3)' }}>
        <div style={{ fontSize: 15, fontWeight: 800, color: COLORS.ink, marginBottom: 4 }}>Voice clip</div>
        <div style={{ fontSize: 12, color: '#998F80', marginBottom: 16 }}>Attach a short note to "{item.label || item.glyph || 'this item'}"</div>

        {item.audio_clip_url && mode === 'choose' && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: COLORS.sand, borderRadius: 10, padding: 10, marginBottom: 14 }}>
            <audio src={item.audio_clip_url} controls style={{ flex: 1, height: 32 }} />
            <button onClick={() => { onRemove(item.id); onClose(); }} style={{ background: 'none', border: 'none', color: COLORS.accent, cursor: 'pointer' }} title="Remove clip">
              <Trash2 size={16} />
            </button>
          </div>
        )}
        ...
        {mode === 'choose' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <button onClick={startRecording} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '12px 0', borderRadius: 12, border: 'none', background: COLORS.accent, color: '#fff', fontSize: 13.5, fontWeight: 700, cursor: 'pointer' }}>
              <Mic size={16} /> Record (max {MAX_SECONDS}s)
            </button>
            <button onClick={() => fileInputRef.current?.click()} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '12px 0', borderRadius: 12, border: `1.5px solid ${COLORS.line}`, background: '#fff', color: COLORS.ink, fontSize: 13.5, fontWeight: 700, cursor: 'pointer' }}>
              <Upload size={16} /> Upload audio file
            </button>
            <input ref={fileInputRef} type="file" accept="audio/*" onChange={handleFilePick} style={{ display: 'none' }} />
          </div>
        )}

        {mode === 'recording' && (
          <div style={{ textAlign: 'center', padding: '10px 0' }}>
            <div style={{ width: 64, height: 64, borderRadius: '50%', background: COLORS.accentSoft, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 14px' }}>
              <div style={{ width: 16, height: 16, borderRadius: 4, background: COLORS.accent, animation: 'pulse 1s infinite' }} />
            </div>
            <div style={{ fontSize: 20, fontWeight: 800, color: COLORS.ink, marginBottom: 14 }}>{seconds}s / {MAX_SECONDS}s</div>
            <button onClick={stopRecording} style={{ padding: '10px 24px', borderRadius: 12, border: 'none', background: COLORS.ink, color: '#fff', fontSize: 13.5, fontWeight: 700, cursor: 'pointer' }}>
              Stop
            </button>
        ...
        {mode === 'reviewing' && recordedUrl && (
          <div>
            <audio src={recordedUrl} controls style={{ width: '100%', marginBottom: 14 }} />
            {error && <div style={{ fontSize: 12, color: COLORS.accent, marginBottom: 10 }}>{error}</div>}
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={saveClip} disabled={saving} style={{ flex: 1, padding: '10px 0', borderRadius: 10, border: 'none', background: COLORS.accent, color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', opacity: saving ? 0.7 : 1 }}>
                {saving ? 'Saving...' : 'Use this clip'}
              </button>
              <button onClick={discardAndRetry} disabled={saving} style={{ padding: '10px 16px', borderRadius: 10, border: `1.5px solid ${COLORS.line}`, background: '#fff', color: COLORS.ink, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                Redo
              </button>
            </div>
          </div>
        ...
        {error && mode === 'choose' && <div style={{ fontSize: 12, color: COLORS.accent, marginTop: 10 }}>{error}</div>}

        <button onClick={onClose} style={{ display: 'block', margin: '14px auto 0', background: 'none', border: 'none', color: '#998F80', fontSize: 12.5, cursor: 'pointer' }}>
          Cancel
        </button>
      </div>
    </div>
...
/** Full audio player UI shown in the book detail "Listen" tab.
 * Reads/writes the shared persistent audio state (see usePersistentAudio)
 * so play position carries over if the user navigates to the mind map
 * and back. */
function AudioPlayer({ audio, book }) {
  useEffect(() => { audio.loadBook(book); }, [book.id]);
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 14, padding: 18, border: `1px solid ${COLORS.line}` }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10 }}>
        <span style={{ fontSize: 11, color: '#998F80', minWidth: 36 }}>{fmtTime(audio.currentTime)}</span>
        <input type="range" min={0} max={audio.duration || 0} value={audio.currentTime} onChange={(e) => audio.seekTo(Number(e.target.value))}
          style={{ flex: 1, accentColor: COLORS.accent }} />
        <span style={{ fontSize: 11, color: '#998F80', minWidth: 36, textAlign: 'right' }}>{fmtTime(audio.duration)}</span>
      </div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 18 }}>
        <button onClick={() => audio.skip(-15)} title="Back 15s" style={{ background: 'none', border: 'none', color: COLORS.ink, cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <Rewind size={20} />
        </button>
        <button onClick={audio.togglePlay} style={{
        ...
        <button onClick={() => audio.skip(15)} title="Forward 15s" style={{ background: 'none', border: 'none', color: COLORS.ink, cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <FastForward size={20} />
        </button>
        <button onClick={audio.cycleRate} title="Playback speed" style={{ background: COLORS.sand, border: 'none', borderRadius: 14, padding: '4px 10px', color: COLORS.ink, cursor: 'pointer', fontSize: 12, fontWeight: 700 }}>
          {audio.rate}x
        </button>
      </div>
    </div>
...
function BookDetailScreen({ auth, book, onBack, onLogout, onOpenMindMap, audio }) {
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: COLORS.cream, fontFamily: "'Poppins', 'Segoe UI', sans-serif" }}>
      <div style={{ padding: '12px 16px', background: '#fff', borderBottom: `1px solid ${COLORS.line}`, display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <button onClick={onOpenMindMap} title="Open mind map" style={{
          display: 'flex', alignItems: 'center', gap: 5, padding: '7px 11px', borderRadius: 20,
          border: 'none', background: COLORS.ink, color: '#fff', fontSize: 12, fontWeight: 700, cursor: 'pointer', flexShrink: 0,
        }}>
          <Map size={14} /> Map
        </button>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
        {tab === 'listen' && <AudioPlayer audio={audio} book={book} />}
        {tab === 'read' && (
          <div style={{ background: '#fff', borderRadius: 14, padding: 18, border: `1px solid ${COLORS.line}` }}>
            {book.pdf_url && (
...
function MindBoardScreen({ auth, book, onBack, onLogout, audio }) {
  ...
  const [clipModalFor, setClipModalFor] = useState(null); // item id, or null when closed
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        setItems((loadedItems || []).map((it) => makeItem({ ...it, x: Number(it.x), y: Number(it.y), w: Number(it.w), h: Number(it.h) })));
  ...
  const saveBoard = async () => {
    ...
    try {
      ...
      if (items.length > 0) {
        await sb.rest('board_items', { method: 'POST', token: auth.token, body: items.map((it) => toSavePayload(it, boardId)) });
      }
  ...
  const startVoiceSearch = () => {
    ...
    rec.onresult = (e) => { setSearch(e.results[0][0].transcript); setPaletteOpen(true); };
    rec.start();
  };
  ...
  const addItem = (partial) => {
    pushHistory();
    const id = nextId();
    const item = makeItem({ id, x: 140 + Math.random() * 160, y: 120 + Math.random() * 160, ...partial });
  ...
  /* NOTE: uploads are intentionally sent ONE AT A TIME (not batched into
     a single bulk insert). This sidesteps PGRST102 entirely for this path —
     a single-row insert can't have a "key mismatch" with other rows. If this
     is ever changed to a bulk insert for performance with many images,
     route each row through a canonical shape first (see ITEM_DEFAULTS /
     toSavePayload pattern for board_items) so every row has identical keys. */
  ...
  const handleHighlight = () => {
    const hasTextSelected = items.some((it) => selectedIds.includes(it.id) && it.type === 'text');
    if (!hasTextSelected) return;
    pushHistory();
    setItems((its) => its.map((it) => (selectedIds.includes(it.id) && it.type === 'text') ? { ...it, highlight: !it.highlight } : it));
  };
  const attachClipToItem = (itemId, clipUrl) => {
    pushHistory();
    setItems((its) => its.map((it) => it.id === itemId ? { ...it, audio_clip_url: clipUrl } : it));
  };
  const removeClipFromItem = (itemId) => {
    pushHistory();
    setItems((its) => its.map((it) => it.id === itemId ? { ...it, audio_clip_url: null } : it));
  };
  const handleCopy = () => {
  ...
  const renderItem = (item) => {
    ...
    return (
      <div key={item.id} style={baseStyle} onMouseDown={(e) => handlePointerDown(e, item)} onTouchStart={(e) => handlePointerDown(e, item)} onDoubleClick={() => startEdit(item)}>
        ...
        {item.audio_clip_url && (
          <button
            onClick={(e) => { e.stopPropagation(); const a = new Audio(item.audio_clip_url); a.play(); }}
            onMouseDown={(e) => e.stopPropagation()}
            onTouchStart={(e) => e.stopPropagation()}
            title="Play voice clip"
            style={{
              position: 'absolute', top: -8, right: -8, width: 22, height: 22, borderRadius: '50%',
              border: '2px solid #fff', background: COLORS.accent, color: '#fff', display: 'flex',
              alignItems: 'center', justifyContent: 'center', cursor: 'pointer', boxShadow: '0 2px 6px rgba(0,0,0,0.25)', zIndex: 5,
            }}
          >
            <Mic size={11} />
          </button>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', width: '100%', fontFamily: "'Poppins', 'Segoe UI', sans-serif", background: COLORS.cream, overflow: 'hidden' }}>
      <div style={{ padding: '10px 14px', background: '#fff', borderBottom: `1px solid ${COLORS.line}`, display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 1, minWidth: 100, position: 'relative', display: 'flex', alignItems: 'center' }}>
          ...
          <input value={search} onChange={(e) => { setSearch(e.target.value); setPaletteOpen(true); }} onFocus={() => setPaletteOpen(true)} placeholder="Search: man, axe, tree..."
        ...
        <span
          onClick={() => { if (saveState === 'error' && saveErrorDetail) alert(saveErrorDetail); }}
          style={{ fontSize: 11, color: saveState === 'error' ? COLORS.accent : '#998F80', minWidth: 50, flexShrink: 0, cursor: saveState === 'error' ? 'pointer' : 'default', textDecoration: saveState === 'error' ? 'underline' : 'none' }}
        >
          {saveState === 'saving' && 'Saving...'}
          {saveState === 'saved' && '✓ Saved'}
          {saveState === 'error' && 'Save failed (tap)'}
        </span>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', background: COLORS.sand, borderBottom: `1px solid ${COLORS.line}`, overflowX: 'auto' }}>
        ...
        <ToolButton onClick={handleHighlight} label="Highlight" disabled={!items.some((it) => selectedIds.includes(it.id) && it.type === 'text')}><Highlighter size={16} /></ToolButton>
        <ToolButton onClick={() => setPaletteOpen(true)} label="Add icon or image" active={paletteOpen}><Plus size={16} /></ToolButton>
        {book.pdf_url && (
          <ToolButton onClick={() => setPdfOpen((p) => !p)} label="View PDF" active={pdfOpen}><FileText size={16} /></ToolButton>
        )}
        <ToolButton onClick={() => setClipModalFor(selectedIds[0])} label="Attach voice clip" disabled={selectedIds.length !== 1}><Mic size={16} /></ToolButton>
        ...
        <ToolButton onClick={handlePaste} label="Paste" disabled={!clipboard.length}><ClipboardList size={16} /></ToolButton>
        ...
        <Divider />
        <span style={{ fontSize: 11.5, color: copyFeedback ? '#2F8F6F' : '#998F80', whiteSpace: 'nowrap', flexShrink: 0, fontWeight: copyFeedback ? 700 : 400 }}>
          {copyFeedback ? '✓ Copied' : connectMode ? (connectFrom ? 'Tap second item' : 'Tap first item') : `${items.length} items`}
        </span>
      </div>

      <MiniAudioBar audio={audio} book={book} />
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden', position: 'relative' }}>
            ...
            <>
              ...
              {items.length === 0 && (
                <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', textAlign: 'center', color: '#B8AC98', pointerEvents: 'none', maxWidth: 280 }}>
                  ...
                  <div style={{ fontSize: 12.5, marginTop: 4 }}>Tap the + button below to add icons, or use a shape above.</div>
                </div>
              )}
              {editingId && (() => {
        ...
        {pdfOpen && book.pdf_url && (
          <DraggablePdfWindow pdfUrl={book.pdf_url} onClose={() => setPdfOpen(false)} />
        )}

        {clipModalFor && itemById(clipModalFor) && (
          <ClipRecorderModal
            item={itemById(clipModalFor)}
            auth={auth}
            onAttach={attachClipToItem}
            onRemove={removeClipFromItem}
            onClose={() => setClipModalFor(null)}
          />
        )}

        {/* Asset drawer — slides up from the bottom as an overlay, doesn't take canvas width */}
        {paletteOpen && (
          <>
            <div onClick={() => setPaletteOpen(false)} style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.15)', zIndex: 25 }} />
            <div style={{
              position: 'absolute', left: 0, right: 0, bottom: 0, maxHeight: '55%', background: '#fff',
              borderTopLeftRadius: 18, borderTopRightRadius: 18, boxShadow: '0 -4px 20px rgba(0,0,0,0.15)',
              zIndex: 30, display: 'flex', flexDirection: 'column', overflow: 'hidden',
            }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px 8px' }}>
                <div style={{ width: 36, height: 4, borderRadius: 2, background: COLORS.line, margin: '0 auto' }} />
              </div>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 16px 10px' }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: '#998F80', textTransform: 'uppercase', letterSpacing: 0.5 }}>
                  {search ? `Results (${filteredAssets.length})` : 'Asset library'}
                </div>
                <button onClick={() => setPaletteOpen(false)} style={{ background: 'none', border: 'none', color: '#998F80', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>
                  Done
                </button>
              </div>
              <div style={{ flex: 1, overflowY: 'auto', padding: '0 16px 16px' }}>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
                  {filteredAssets.slice(0, 60).map((a) => (
                    <button key={a.id} onClick={() => addEmoji(a)} title={a.tags.join(', ')} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '10px 4px', borderRadius: 10, border: `1px solid ${COLORS.line}`, background: COLORS.sand, cursor: 'pointer', gap: 3 }}>
                      <span style={{ fontSize: 26 }}>{a.glyph}</span>
                      <span style={{ fontSize: 9.5, color: '#8A8070', textAlign: 'center', lineHeight: 1.1 }}>{a.tags[0]}</span>
                    </button>
                  ))}
                  {filteredAssets.length === 0 && <div style={{ fontSize: 12, color: '#B8AC98', gridColumn: '1 / -1', padding: 14, textAlign: 'center' }}>No matches.</div>}
                </div>
                {uploadedImages.length > 0 && (
                  <>
                    <div style={{ fontSize: 11, fontWeight: 700, color: '#998F80', margin: '16px 0 8px', textTransform: 'uppercase', letterSpacing: 0.5 }}>Your uploads</div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
                      {uploadedImages.map((img) => (
                        <button key={img.id} onClick={() => addImage(img.src, img.name)} title={img.name} style={{ padding: 0, borderRadius: 10, border: `1px solid ${COLORS.line}`, overflow: 'hidden', cursor: 'pointer', height: 64, background: '#fff' }}>
...
export default function MindBoardRoot() {
  ...
  let activeScreen;
  if (screen === 'admin') {
    activeScreen = <AdminPanel auth={auth} onBack={() => setScreen('library')} />;
  } else if (screen === 'mindmap' && selectedBook) {
    activeScreen = <MindBoardScreen auth={auth} book={selectedBook} onBack={() => setScreen('bookdetail')} onLogout={handleLogout} audio={audio} />;
  } else if (screen === 'bookdetail' && selectedBook) {
    activeScreen = (
      <BookDetailScreen
        auth={auth}
        book={selectedBook}
